...
type TagFormDefaults = Pick<NewTag, 'id' | 'clientes'>;

type TagFormGroupContent = {
  id: FormControl<ITag['id'] | NewTag['id']>;
  nome: FormControl<ITag['nome']>;
  clientes: FormControl<ITag['clientes']>;
};
...
export class TagFormService {
  createTagFormGroup(tag: TagFormGroupInput = { id: null }): TagFormGroup {
    ...
    return new FormGroup<TagFormGroupContent>({
      ...
      nome: new FormControl(tagRawValue.nome, {
        validators: [Validators.required],
      }),
      clientes: new FormControl(tagRawValue.clientes ?? []),
    });
  }
