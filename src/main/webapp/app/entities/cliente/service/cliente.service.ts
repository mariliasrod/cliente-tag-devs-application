...
type RestOf<T extends ICliente | NewCliente> = Omit<T, 'dataNascimento'> & {
...
export class ClienteService {
  ...
  protected convertDateFromClient<T extends ICliente | NewCliente | PartialUpdateCliente>(cliente: T): RestOf<T> {
    return {
      ...cliente,
      dataNascimento: cliente.dataNascimento?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCliente: RestCliente): ICliente {
    return {
      ...restCliente,
      dataNascimento: restCliente.dataNascimento ? dayjs(restCliente.dataNascimento) : undefined,
    };
  }
